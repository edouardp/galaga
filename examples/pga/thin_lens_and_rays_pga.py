...
def _(coords, focal_length, gm, object_distance, object_height, point):
    ...
    Object tip {object_tip} = {object_tip:value}

    Lens center {lens_center} = {lens_center:value}

    Right focal point {focus_right} = {focus_right:value}

    Image tip {image_tip} = {image_tip:value}
...
def _(focal_length, image_tip, lens_center, np, object_distance, object_height, plt, sp):
    _s = object_distance.value
    _f = focal_length.value
    _h = object_height.value
    _fig, _ax = plt.subplots(figsize=(10, 4.5))

    _ax.axvline(0, color="black", linewidth=2, alpha=0.5)
    _ax.plot([-_s, -_s], [0, _h], color="steelblue", linewidth=3, label="object")
    if np.isfinite(sp):
        _img = image_tip
        _ix = _img.data[14]
        _iy = -_img.data[13]
        _ax.plot([_ix, _ix], [0, _iy], color="crimson", linewidth=3, label="image")
    _ax.plot([-_f, _f], [0, 0], "ko", ms=6)
    _ax.text(-_f, -0.25, "F", ha="center")
    _ax.text(_f, -0.25, "F'", ha="center")

    _ax.plot([-_s, 0], [_h, _h], color="darkorange", linewidth=2)
    if np.isfinite(sp):
        _ax.plot([0, sp], [_h, 0], color="darkorange", linewidth=2)
        _ax.plot([-_s, sp], [_h, 0], color="seagreen", linewidth=2)

    _ax.set_xlim(-max(1.5 * _s, 2), max(1.5 * (sp if np.isfinite(sp) else _s), 4))

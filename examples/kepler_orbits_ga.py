import marimo

__generated_with = "0.21.1"
app = marimo.App()


@app.cell
def _():
    import sys
    from pathlib import Path

    _root = str(Path(__file__).resolve().parent.parent)
    _gamo = str(Path(__file__).resolve().parent.parent / "packages" / "gamo")
    for p in [_root, _gamo]:
        if p not in sys.path:
            sys.path.insert(0, p)
    return


@app.cell
def _():
    import marimo as mo
    import numpy as np
    import matplotlib.pyplot as plt
    import matplotlib

    matplotlib.rcParams.update({"figure.facecolor": "white"})

    from ga import Algebra, norm
    import galaga_marimo as gm

    return Algebra, gm, mo, norm, np, plt


@app.cell(hide_code=True)
def _(mo):
    mo.md(
        """
        # Kepler Orbits with Geometric Algebra

        The Kepler problem is naturally planar. Geometric algebra makes the angular
        momentum plane and the oriented area structure explicit instead of hiding them
        inside cross products and coordinate determinants.
        """
    )
    return


@app.cell
def _(Algebra):
    alg = Algebra((1, 1, 1), repr_unicode=True)
    e1, e2, e3 = alg.basis_vectors(lazy=True)
    return e1, e2, e3


@app.cell
def _(mo):
    eccentricity = mo.ui.slider(start=0.0, stop=0.95, step=0.01, value=0.45, label="eccentricity")
    semilatus = mo.ui.slider(start=1.0, stop=10.0, step=0.1, value=4.0, label="semilatus rectum")
    anomaly = mo.ui.slider(start=0, stop=360, step=1, value=80, label="true anomaly")
    mo.vstack([eccentricity, semilatus, anomaly])
    return anomaly, eccentricity, semilatus


@app.cell(hide_code=True)
def _(anomaly, e1, e2, gm, np, semilatus):
    _theta = np.radians(anomaly.value)
    _r = semilatus.value / (1 + 0.45 * np.cos(_theta))
    _position = _r * (np.cos(_theta) * e1 + np.sin(_theta) * e2)
    _plane = e1 ^ e2
    gm.md(t"""
    ## Orbital Plane

    {e1 ^ e2} = {(e1 ^ e2).eval()}

    Current orbital position:

    {_position} = {_position.eval()}

    The orbit lives in the bivector plane {_plane} = {_plane.eval()}.
    """)
    return


@app.cell(hide_code=True)
def _(anomaly, e1, e2, eccentricity, gm, norm, np, semilatus):
    _e = eccentricity.value
    _p = semilatus.value
    _theta = np.radians(anomaly.value)
    _r = _p / (1 + _e * np.cos(_theta))
    _position = _r * (np.cos(_theta) * e1.eval() + np.sin(_theta) * e2.eval())
    _velocity = (-np.sin(_theta) * e1.eval() + (_e + np.cos(_theta)) * e2.eval()) / np.sqrt(_p)
    _L = _position ^ _velocity

    gm.md(t"""
    Position vector:

    {_position} = {_position.eval()}

    Velocity vector:

    {_velocity} = {_velocity.eval()}

    Angular momentum bivector:

    {_L} = {_L.eval()}

    Its magnitude is {norm(_L.eval()):.6f}.
    """)
    return


@app.cell
def _(anomaly, eccentricity, np, plt, semilatus):
    _e = eccentricity.value
    _p = semilatus.value
    _ths = np.linspace(0, 2 * np.pi, 500)
    _rs = _p / (1 + _e * np.cos(_ths))
    _x = _rs * np.cos(_ths)
    _y = _rs * np.sin(_ths)

    _th = np.radians(anomaly.value)
    _r = _p / (1 + _e * np.cos(_th))
    _xp = _r * np.cos(_th)
    _yp = _r * np.sin(_th)

    _fig, (_ax1, _ax2) = plt.subplots(1, 2, figsize=(11, 4.5))
    _ax1.plot(_x, _y, color="steelblue", linewidth=2.5)
    _ax1.plot([0], [0], "yo", ms=10)
    _ax1.plot([_xp], [_yp], "ko", ms=7)
    _ax1.set_aspect("equal")
    _ax1.set_title("Kepler orbit in configuration space")
    _ax1.grid(True, alpha=0.2)

    _vx = -np.sin(_th) / np.sqrt(_p)
    _vy = (_e + np.cos(_th)) / np.sqrt(_p)
    _ths2 = np.linspace(0, 2 * np.pi, 500)
    _vxs = -np.sin(_ths2) / np.sqrt(_p)
    _vys = (_e + np.cos(_ths2)) / np.sqrt(_p)
    _ax2.plot(_vxs, _vys, color="crimson", linewidth=2.5)
    _ax2.plot([0], [_e / np.sqrt(_p)], "yo", ms=10)
    _ax2.plot([_vx], [_vy], "ko", ms=7)
    _ax2.set_aspect("equal")
    _ax2.set_title("Velocity-space hodograph")
    _ax2.grid(True, alpha=0.2)
    _fig.tight_layout()
    _fig


@app.cell
def _():
    return


if __name__ == "__main__":
    app.run()

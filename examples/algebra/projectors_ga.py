...
def _():
    ...
    from galaga import Algebra, DisplayPolicy, exp, inverse, left_contraction
...
def _(R, e1, e3, gm, inverse, left_contraction, v):
    ...
    {p_plane + r_plane}
    """)
    return B, p_plane, r_plane


@app.cell
def _(R, e1, e3, np, p_plane, plt, r_plane, v):
    _v = v.vector_part
    ...
    # Transport the reference plane's spanning vectors by the same rotor
    # used for B. The plotted surface must represent that computed blade.
    plane_spanners = np.column_stack(((R * e1 * ~R).vector_part, (R * e3 * ~R).vector_part))
    _s, _t = np.meshgrid(np.linspace(-2.5, 2.5, 2), np.linspace(-2.5, 2.5, 2))
    plane_mesh = plane_spanners[:, 0, None, None] * _s + plane_spanners[:, 1, None, None] * _t
    _ax.plot_surface(*plane_mesh, alpha=0.12, color="steelblue")
    ...
    _ax.set_title("Projection into the rotated e₁∧e₃ plane")
    _fig.tight_layout()
    _fig
    return plane_mesh, plane_spanners


@app.cell(hide_code=True)
def _(mo):
    mo.md(r"""
    ## Metric and invertibility matter

    The formulas require the blade to be invertible: equivalently, the Gram
    matrix restricted to its spanning subspace must be nonsingular. The
    ambient metric may still be degenerate. A null line has no such inverse;
    silently using a pseudoinverse would define a different projection.

    For spanning columns $C$ and Gram matrix $G$, an independent coordinate
    check is $P=C(C^TGC)^{-1}C^TG$. It gives $P^2=P$ and
    $C^TG(v-Pv)=0$, without relying on Euclidean dot products in an oblique
    or indefinite frame. The plots above use a Euclidean metric and the
    actual transported spanning vectors, not a fixed coordinate plane.
    """)
    return

...
Convention note — commutator product:
    ...
    In Galaga 2 this is ``half_commutator(A, B)``, not the unscaled
    ``commutator(A, B)`` or ``lie_bracket(A, B)``. All bracket tests in this
    file use ``half_commutator`` to match the paper exactly.
"""
...
def alg(request):
    return Algebra(signature=request.param)
...
class TestDualOfProduct:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            B = _rmv(alg, rng)
            lhs = dual(geometric_product(A, B))
            rhs = geometric_product(A, dual(B))
...
# ---------------------------------------------------------------------------
# Eq. 2.54 (label:commuteI) — A·I = I·grade_involution^{n-1}(A)
# ---------------------------------------------------------------------------


class TestEq254PseudoscalarCommutation:
    """Eq. 2.54 (§5.5): A·I = I·grade_involution^{n-1}(A)."""
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            lhs = geometric_product(A, I)
            # The power is grade_involution if n-1 is odd, identity otherwise.
            A_inv = grade_involution(A) if (alg.n - 1) % 2 == 1 else A
            rhs = geometric_product(I, A_inv)
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestBladeWedgeDualProportionalToI:
    ...
    def test_basis_blades(self, alg):
        ...
        I = alg.pseudoscalar()
        I2 = float(geometric_product(I, I))
        for k in range(1, alg.n):
            for B in alg.basis_blades(k):
                B2 = float(geometric_product(B, B))
                if abs(B2) < 1e-10:
                    continue
                result = outer_product(B, dual(B))
                expected = (B2 / I2) * I
...
#   - half_commutator(A, B) = ½(AB − BA) ← matches Chisolm, used here
#   - commutator(A, B) = lie_bracket(A, B) = AB − BA
# ===========================================================================

# ---------------------------------------------------------------------------
# Eq. 2.62 (label:commident) — [A, BC] = [A,B]C + B[A,C]  (Leibniz rule)
# Uses half_commutator = ½(AB−BA), matching Chisolm's [A,B].
# ---------------------------------------------------------------------------
...
class TestEq262CommutatorLeibniz:
    ...
    Uses Galaga's half_commutator, which matches Chisolm's scaled commutator.
    """
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            ...
            B = _rmv(alg, rng)
            C = _rmv(alg, rng)
            lhs = half_commutator(A, geometric_product(B, C))
            rhs = geometric_product(half_commutator(A, B), C) + geometric_product(B, half_commutator(A, C))
...
class TestJacobiIdentity:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            ...
            B = _rmv(alg, rng)
            C = _rmv(alg, rng)
            t1 = half_commutator(A, half_commutator(B, C))
            t2 = half_commutator(B, half_commutator(C, A))
            t3 = half_commutator(C, half_commutator(A, B))
            result = t1 + t2 + t3
...
class TestBivectorCommutatorPreservesGrade:
    ...
    def test_equals_grade_r_of_product(self, alg):
        ...
        for r in range(alg.n + 1):
            Br = _rblade(alg, rng, r)
            comm = half_commutator(A2, Br)
            # half_commutator = (A2*Br - Br*A2)/2, and the grade-r part of that
            # equals the grade-r part of A2*Br (since inner/outer parts cancel)
            expected = grade(geometric_product(A2, Br), r)
...
# Uses half_commutator = ½(aA−Aa), matching Chisolm's [a,A].
# ---------------------------------------------------------------------------
...
class TestEq265VectorCommutatorDecomposition:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            ...
            A_even = grade(A, "even")
            A_odd = grade(A, "odd")
            lhs = half_commutator(a, A)
            rhs = left_contraction(a, A_even) + outer_product(a, A_odd)
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestSkewSymmetricBivectorOperator:
    ...
    def test_antisymmetric_bilinear_form(self, alg):
        ...
        for _ in range(5):
            a = _rvec(alg, rng)
            b = _rvec(alg, rng)
            lhs = float(scalar_product(a, left_contraction(b, A2)))
            rhs = -float(scalar_product(b, left_contraction(a, A2)))
    ...
    def test_equals_wedge_contracted(self, alg):
        ...
        for _ in range(5):
            ...
            b = _rvec(alg, rng)
            lhs = float(scalar_product(a, left_contraction(b, A2)))
            rhs = float(left_contraction(outer_product(a, b), A2))
            assert lhs == pytest.approx(rhs, abs=1e-10)
    ...
    def test_reconstruct_bivector(self, alg):
        ...
        for i, ei in enumerate(vecs):
            ...
            reconstructed = reconstructed + outer_product(ei_recip, F_ei)

...
class TestAxiom4VectorSquareIsScalar:
    ...
    def test_basis_vectors(self, alg):
        ...
        for i, e in enumerate(alg.basis_vectors()):
            v2 = geometric_product(e, e)
            assert is_scalar(v2), f"e{i + 1}² is not scalar"
            assert float(v2) == pytest.approx(alg.signature[i])
    ...
    def test_random_vectors(self, alg):
        ...
        for _ in range(5):
            ...
            assert is_scalar(geometric_product(v, v))
...
class TestSymmetrizedProduct:
    ...
    def test_basis_pairs(self, alg):
        ...
        for i, ei in enumerate(vecs):
            for j, ej in enumerate(vecs):
                sym = 0.5 * (geometric_product(ei, ej) + geometric_product(ej, ei))
                expected = alg.signature[i] if i == j else 0.0
                assert is_scalar(sym)
                assert float(sym) == pytest.approx(expected)
    ...
    def test_random_pairs(self, alg):
        ...
        for _ in range(5):
            ...
            v = _random_vector(alg, rng)
            sym = 0.5 * (geometric_product(u, v) + geometric_product(v, u))
            assert is_scalar(sym)
...
class TestGPDecomposition:
    ...
    def test_random_vectors(self, alg):
        ...
        for _ in range(5):
            u = _random_vector(alg, rng)
            v = _random_vector(alg, rng)
            lhs = geometric_product(u, v)
            rhs = left_contraction(u, v) + outer_product(u, v)
...
class TestThm2OuterProductDependence:
    ...
    def test_self_wedge_vanishes(self, alg):
        ...
        v = _random_vector(alg, rng)
        assert np.allclose(outer_product(v, v).data, 0, atol=1e-12)
    ...
    def test_dependent_triple_vanishes(self, alg):
        ...
        assert np.allclose(outer_product(outer_product(c, a), b).data, 0, atol=1e-12)

    def test_independent_nonzero(self, alg):
        """Outer product of independent basis vectors is nonzero."""
        vecs = alg.basis_vectors()
        if len(vecs) >= 2:
            result = outer_product(vecs[0], vecs[1])
...
class TestThm3BladeSubspace:
    ...
    def test_vector_in_blade_wedges_to_zero(self, alg):
        ...
        assert np.allclose(outer_product(vecs[0], B).data, 0, atol=1e-12)
        assert np.allclose(outer_product(vecs[1], B).data, 0, atol=1e-12)
    ...
    def test_vector_outside_blade_nonzero(self, alg):
        ...
        B = outer_product(vecs[0], vecs[1])
        assert not np.allclose(outer_product(vecs[2], B).data, 0)
    ...
    def test_linear_combination_in_blade(self, alg):
        ...
        assert np.allclose(outer_product(combo, B).data, 0, atol=1e-12)
...
class TestThm4SameSubspaceScalarMultiple:
    ...
    def test_rescaled_blade(self, alg):
        ...
        if len(vecs) < 2:
            pytest.skip("Need n ≥ 2")
        B1 = outer_product(vecs[0], vecs[1])
        B2 = 5.0 * B1
        # Same subspace: any vector in B1 also wedges to zero with B2
        assert np.allclose(outer_product(vecs[0], B2).data, 0, atol=1e-12)
        assert np.allclose(outer_product(vecs[1], B2).data, 0, atol=1e-12)
    ...
    def test_reordered_factors(self, alg):
        ...
        if len(vecs) < 2:
            pytest.skip("Need n ≥ 2")
        B1 = outer_product(vecs[0], vecs[1])
        B2 = outer_product(vecs[1], vecs[0])
    ...
    def test_orthogonal_basis(self, alg):
        ...
        for i in range(len(vecs)):
            for j in range(i + 1, len(vecs)):
                w = outer_product(vecs[i], vecs[j])
                g = geometric_product(vecs[i], vecs[j])
...
class TestThm5OrthogonalityViaContraction:
    ...
    def test_orthogonal_vector_contracts_to_zero(self, alg):
        ...
        if len(vecs) < 3:
            pytest.skip("Need n ≥ 3")
        B = outer_product(vecs[0], vecs[1])  # blade in e1-e2 plane
...
class TestVectorInverse:
    ...
    def test_basis_inverse(self, alg):
        ...
        for i, e in enumerate(alg.basis_vectors()):
            ...
            v_inv = inverse(e)
            product = geometric_product(e, v_inv)
    ...
    def test_random_nonnull_inverse(self, alg):
        ...
        for _ in range(5):
            v = _random_vector(alg, rng)
            v2 = float(geometric_product(v, v))
            if abs(v2) < 1e-10:
                continue
            v_inv = inverse(v)
            product = geometric_product(v, v_inv)
            assert float(product) == pytest.approx(1.0, abs=1e-12)

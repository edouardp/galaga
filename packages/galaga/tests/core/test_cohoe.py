...
def _rversor(alg, rng, k):
    ...
    for _ in range(k - 1):
        result = geometric_product(result, _rvec(alg, rng))
    return result
...
class TestThm319GeneralisedAnticommutator:
    """Thm 3.19: aX - grade_involution(X)a = 2(a⌋X) for vector a and multivector X."""
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            a = _rvec(alg, rng)
            X = _rmv(alg, rng)
            lhs = geometric_product(a, X) - geometric_product(grade_involution(X), a)
...
class TestThm318InvoluteContraction:
    """Thm 3.18: grade_involution(a⌋X) = -a⌋grade_involution(X) for vector a."""
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            a = _rvec(alg, rng)
            X = _rmv(alg, rng)
            lhs = grade_involution(left_contraction(a, X))
            rhs = -1.0 * left_contraction(a, grade_involution(X))
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestSec52ContractionInvolutionIdentities:
    ...
    def test_involute_left_contraction(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            B = _rmv(alg, rng)
            lhs = grade_involution(left_contraction(A, B))
            rhs = left_contraction(grade_involution(A), grade_involution(B))
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
    ...
    def test_involute_right_contraction(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            B = _rmv(alg, rng)
            lhs = grade_involution(right_contraction(A, B))
            rhs = right_contraction(grade_involution(A), grade_involution(B))
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestThm411ScalarPartInvariantUnderReverse:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            assert float(grade(reverse(A), 0)) == pytest.approx(float(grade(A, 0)), abs=1e-12)
...
class TestThm49GradeProjectionCommutesWithInvolutions:
    ...
    def test_involute(self, alg):
        ...
        for k in range(alg.n + 1):
            lhs = grade_involution(grade(A, k))
            rhs = grade(grade_involution(A), k)
    ...
    def test_random(self, alg):
        ...
        for _ in range(3):
            ...
            for n in range(1, alg.n + 1):
                X = _rblade(alg, rng, n)
                lhs = grade(geometric_product(a, X), n - 1)
...
class TestThm56RightProductDecomposition:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            X = _rmv(alg, rng)
            v = _rvec(alg, rng)
            lhs = geometric_product(X, v)
            rhs = right_contraction(X, v) + outer_product(X, v)
            assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestSec52VectorInnerProductsAgree:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            ...
            # Both should be the vector-space bilinear form a^T G b.
            a_coefficients = np.array([a.data[1 << i] for i in range(alg.n)])
            b_coefficients = np.array([b.data[1 << i] for i in range(alg.n)])
            expected = float(a_coefficients @ alg.gram @ b_coefficients)
            assert float(lc) == pytest.approx(expected, abs=1e-12)
    ...
    def test_random(self, alg):
        ...
        for r in range(min(alg.n, 3) + 1):
            for s in range(min(alg.n, 3) + 1):
                ...
                A = _rblade(alg, rng, r)
                B = _rblade(alg, rng, s)
                assert float(scalar_product(A, B)) == pytest.approx(0, abs=1e-10)
...
class TestThm54ScalarProductSymmetry:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            B = _rmv(alg, rng)
            assert float(scalar_product(A, B)) == pytest.approx(float(scalar_product(B, A)), abs=1e-10)
...
class TestThm55NormInvariantUnderReverse:
    ...
    def test_random(self, alg):
        ...
        for _ in range(5):
            A = _rmv(alg, rng)
            assert float(norm2(reverse(A))) == pytest.approx(float(norm2(A)), abs=1e-10)
    ...
    def test_random_versors(self, alg):
        ...
        for k in range(1, min(alg.n, 4) + 1):
            for _ in range(3):
                ...
                Vr = reverse(V)
                lhs1 = geometric_product(Vr, V)
                lhs2 = geometric_product(V, Vr)
                n2 = float(norm2(V))
    ...
    def test_random_versors(self, alg):
        ...
        for k in range(1, min(alg.n, 3) + 1):
            for _ in range(3):
                ...
                sX = sandwich(A, X)
                sY = sandwich(A, Y)
                lhs = outer_product(sX, sY)
                rhs = n2 * sandwich(A, outer_product(X, Y))

...
class TestDoubleGradeInvolution:
    """After Eq. 2.3 (§5.1): applying grade_involution twice returns A."""

    def test_random(self, alg):
        """Double grade involution is identity."""
        rng = np.random.default_rng(71)
        A = _rmv(alg, rng)
        assert np.allclose(grade_involution(grade_involution(A)).data, A.data, atol=1e-12)
...
class TestGradeInvolutionProduct:
    """Eq. 2.1 (§5.1): grade_involution(AB) equals grade_involution(A) grade_involution(B)."""
    ...
    def test_random(self, alg):
        ...
        A = _rmv(alg, rng)
        B = _rmv(alg, rng)
        lhs = grade_involution(geometric_product(A, B))
        rhs = geometric_product(grade_involution(A), grade_involution(B))
...
class TestEq212ReversionOfProduct:
    ...
    def test_random(self, alg):
        ...
        A = _rmv(alg, rng)
        B = _rmv(alg, rng)
        lhs = reverse(geometric_product(A, B))
        rhs = geometric_product(reverse(B), reverse(A))
        assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
# ---------------------------------------------------------------------------
# Clifford conjugation = grade_involution(reverse(A))
# ---------------------------------------------------------------------------


class TestCliffordConjIsInvoluteReverse:
    """§5.3: conjugate(A) = grade_involution(reverse(A))."""

    def test_random(self, alg):
        """Clifford conjugation equals grade involution composed with reversion."""
        rng = np.random.default_rng(78)
        A = _rmv(alg, rng)
        assert np.allclose(conjugate(A).data, grade_involution(reverse(A)).data, atol=1e-12)
...
class TestEq225ScalarPartCommutes:
    ...
    def test_random(self, alg):
        ...
        A = _rmv(alg, rng)
        B = _rmv(alg, rng)
        lhs = float(grade(geometric_product(A, B), 0))
        rhs = float(grade(geometric_product(B, A), 0))
...
class TestEq226CyclicScalarPart:
    ...
    def test_three_factors(self, alg):
        ...
        B = _rmv(alg, rng)
        C = _rmv(alg, rng)
        abc = float(grade(geometric_product(geometric_product(A, B), C), 0))
        cab = float(grade(geometric_product(geometric_product(C, A), B), 0))
        bca = float(grade(geometric_product(geometric_product(B, C), A), 0))
...
class TestEq230ScalarProductSymmetry:
    ...
    def test_random(self, alg):
        ...
        A = _rmv(alg, rng)
        B = _rmv(alg, rng)
        lhs = float(grade(geometric_product(reverse(A), B), 0))
        rhs = float(grade(geometric_product(reverse(B), A), 0))
        assert lhs == pytest.approx(rhs, abs=1e-10)
...
class TestEq229ScalarProductGradeInvolution:
    ...
    def test_random(self, alg):
        ...
        B = _rmv(alg, rng)
        lhs = float(grade(geometric_product(reverse(A), B), 0))
        rhs = float(grade(geometric_product(reverse(grade_involution(A)), grade_involution(B)), 0))
        assert lhs == pytest.approx(rhs, abs=1e-10)
    ...
    def test_different_grades(self, alg):
        ...
        for r in range(min(alg.n, 3) + 1):
            for s in range(min(alg.n, 3) + 1):
                ...
                sp = float(grade(geometric_product(Ar, Bs), 0))
    ...
    def test_random(self, alg):
        ...
        for r in range(1, min(alg.n, 3) + 1):
            for s in range(1, min(alg.n, 3) + 1):
                Ar = _rblade(alg, rng, r)
                Bs = _rblade(alg, rng, s)
                AB = geometric_product(Ar, Bs)
                BA = geometric_product(Bs, Ar)
...
class TestEq220ReversionOfProducts:
    ...
    def test_reversion_of_outer_product(self, alg):
        ...
        A = _rmv(alg, rng)
        B = _rmv(alg, rng)
        lhs = reverse(outer_product(A, B))
        rhs = outer_product(reverse(B), reverse(A))
        assert np.allclose(lhs.data, rhs.data, atol=1e-10)
...
class TestThm14VersorInverse:
    ...
    def test_rotor_inverse(self, alg):
        ...
        R = geometric_product(nonnull[0], nonnull[1])  # biversor
        n2 = float(norm2(R))
        if abs(n2) < 1e-10:
            pytest.skip("Null rotor")
        R_inv_formula = reverse(R) / n2
        product = geometric_product(R, R_inv_formula)
        assert float(product) == pytest.approx(1.0, abs=1e-12)
...
class TestThm13VersorNormProduct:
    ...
    def test_vector_product(self, alg):
        ...
        for _ in range(5):
            a = _rvec(alg, rng)
            b = _rvec(alg, rng)
            lhs = float(norm2(geometric_product(a, b)))
            rhs = float(norm2(a)) * float(norm2(b))
            assert lhs == pytest.approx(rhs, abs=1e-10)

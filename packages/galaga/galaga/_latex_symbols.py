"""Implementation of the bounded symbol converter exported by galaga.names.
...
common symbols, operators, relations, and arrows. This is not a TeX parser.
...
# Greek lowercase: (latex_cmd, unicode_char)
_GREEK_LOWER = [
    ("alpha", "α"),
    ("beta", "β"),
    ("gamma", "γ"),
...
# Only ASCII Latin letters may use Latin-alphabet offsets. Unicode \w would
# also admit Greek, letterlike symbols, digits and underscores.
_MATH_FONT_RE = re.compile(r"\\(mathbf|mathit|mathcal|mathfrak|mathbb)\{([A-Za-z0-9])\}")
_ACCENT_RE = re.compile(r"\\(hat|tilde|bar|vec|dot|ddot)\{([A-Za-z0-9])\}")
_FONT_LETTERS: dict[str, tuple[int, int, dict[str, str]]] = {
    "mathbf": (_MATHBF_UPPER, _MATHBF_LOWER, {}),
    "mathit": (_MATHIT_UPPER, _MATHIT_LOWER, {"h": "ℎ"}),
    "mathcal": (_MATHCAL_UPPER, _MATHCAL_LOWER, _MATHCAL_SPECIAL),
    "mathfrak": (_MATHFRAK_UPPER, _MATHFRAK_LOWER, _MATHFRAK_SPECIAL),
    "mathbb": (_MATHBB_UPPER, _MATHBB_LOWER, _MATHBB_SPECIAL),
}
_FONT_DIGITS = {"mathbf": 0x1D7CE, "mathbb": 0x1D7D8}
...
class LatexSymbols:
    """Exact single-symbol LaTeX lookup, returning None for unsupported text.

    Supports the explicit command tables, five math fonts on one ASCII Latin
    letter, bold/double-struck ASCII digits, and six accents on one ASCII
    letter or digit. Nested commands, arbitrary macros, non-ASCII font/accent
    bodies and multi-character bodies need explicit presentation spellings.
    No surrounding whitespace is stripped and no Unicode normalization occurs.
    """
    ...
    def _math_font(self, latex: str) -> tuple[str, str] | None:
        ...
        if char in "0123456789":
            start = _FONT_DIGITS.get(font)
            return None if start is None else (chr(start + ord(char) - ord("0")), char)

        upper, lower, special = _FONT_LETTERS[font]
        if char in special:
            uni = special[char]
        elif char.isupper():
            uni = chr(upper + ord(char) - ord("A"))
        else:
            uni = chr(lower + ord(char) - ord("a"))
        return uni, char
    ...
    def lookup(self, latex: str) -> tuple[str, str] | None:
        """Return (unicode, ascii), or None for an unsupported string.
        ...
        if latex in self._unicode:
            return self._unicode[latex], self._ascii[latex]
        return self._math_font(latex) or self._accent(latex)

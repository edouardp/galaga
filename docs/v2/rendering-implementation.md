...
### Concrete render documents

Optional consumers can use `galaga.rendering.value_document(value,
presentation=None)` to obtain the normal concrete value tree together with
semantic anchors. The immutable document captures the resolved presentation
and an ordered tuple of visible term, coefficient-magnitude, sign, and blade
anchors. These records are separate from the tree and carry native blade
masks, derived grades, and displayed orientations.

```python
from galaga.rendering import value_document

document = value_document(A)
bivectors = document.select("term", grade=2)
...
placeholder has no component anchors.

These locations belong to one document. Rebuild the document after changing
presentation instead of storing its indices as reusable selection identities.
The extension may store semantic rules and resolve them against new documents.
The concrete API covers facade values. See
[ADR-143](../adrs/143-concrete-render-documents-and-semantic-anchors.md).

### Expression render documents

`expression_document(expr, presentation, target=None)` builds the ordinary
expression tree with separate immutable `expression_anchors`. Selection uses
the original binary provenance paths, even after simplification and visual
flattening:

```python
from galaga.rendering import expression_document

document = expression_document(A.expr, A.algebra.presentation, target="latex")
products = document.select_expression("operator", operation_id="geometric_product")
left_operand = document.select_expression(path=(0,))
```

An anchor's `render_path` consists of dataclass field names and tuple indices
addressing its `node` within the document body. Repeated displayed uses have
different render paths even when they reuse the same node. `start` and `stop`
identify a half-open interval of terms, factors, or operands when flattening
has removed the original layout container. Rebuild locations after changing
presentation.

Operator `part` values identify function names, symbols, accents, signs, or
separators. `operator_indices` identify the term/factor/operand following a
join, rather than every join in a flattened container. A juxtaposed product
has `part="body"` and `implicit=True`: its annotation targets that product's
extent. Structural fraction/wrapper/script notation initially has
`part="notation"`, selecting the whole notation extent.

Source mapping preserves surviving operands and skips removed operators.
Whole-expression paths may address their simplified result, but eliminated
operations do not claim a surviving same-named inner operation. For example,
the outer addition in `(a + b) + 0` has no operator anchor; the original inner
addition keeps its own. Valid selectors with no visible matches return `()`.

Expression occurrence mapping is recorded in
[ADR-144](../adrs/144-expression-render-occurrence-anchors.md).

### Components within expression operands

Scalar, blade, and multivector literals also supply component anchors with
`scope="expr"` and their original `expression_path`. Result anchors retain
`scope="value"`, the default selection scope:

```python
result_bivectors = document.select("term", grade=2)
left_operand_vectors = document.select("term", scope="expr", path=(0,), grade=1)
operand_coefficients = document.select("coefficient", scope="expr", path=(0,))
```

Path selection includes descendant literal occurrences: a left operand that
is a sum of literals can be selected as one subtree. The empty path `()`
selects the whole expression, while omitted `path` selects all expression
components. Component masks remain native identities regardless of labels
or display order. `render_path` addresses the final combined document, and
`term_index` follows actual flattened sum positions. Signs introduced by
surrounding subtraction are visible slots as well.

Removed literals have no component anchors. A scalar generated by folding
belongs to the folded occurrence, not to its removed inputs. Unit
coefficients, hidden terms, and zero placeholders remain absent. Nonzero
scalar literals follow their existing display policy rather than
multivector zero tolerance. Named symbols remain opaque; the renderer does
not evaluate or expand them to infer components. See
[ADR-146](../adrs/146-expression-component-anchor-scopes.md).

### Teaching content and captured presenters

`content_document(value, content=None, presentation=None, notation=None,
target=None)` combines visible name, expression and value parts with their
anchors. It follows the ordinary display policy, including `auto` and the
fallback when a name or expression is unavailable. Presenter views provide
the same operation with their captured presentation:

```python
view = presets.presenters.lengyel()(A)
document = view.render_document(content="full", target="latex")
operators = document.select_expression("operator")
bivectors = document.select("term", grade=2)
equalities = document.select_content("relation")
```

`select_content` accepts `name`, `expr`, `value`, or `relation`. Component and
expression `render_path` values address the combined document body. Original
expression occurrence paths stay relative to the recorded expression.
Relation anchors reference the equality container and use `relation_index`
to identify the following visible part.

Teaching documents retain only the parts that the ordinary emitter displays.
If expression and result look identical, the first part wins; the suppressed
result has no component anchors. An annotation is not reassigned to another
part because their output matches. Value-only content explicitly exposes
result components. No relation anchors remain when one part survives.

Visibility depends on the output target, captured in `document.target`.
